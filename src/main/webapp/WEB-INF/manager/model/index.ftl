	...
	<@ms.modal modalName="addEditModel" title="添加模块">
		<@ms.modalBody height="350">
			<@ms.form isvalidation=true name="addEditForm"  action="" method="post"  >
				...
				<@ms.text name="modelCode"  label="编码" title="模块编码" placeholder="请输入模块编码" value="" maxlength="20" validation={"required":"true", "data-bv-notempty-message":"请输入模块编码!","data-bv-regexp":"true","data-bv-regexp-regexp":'^[A-Za-z0-9]+$',"data-bv-regexp-message":"模块编码只能为英文字母，数字!"} />
				...
				<@ms.text name="isChild"  label="菜单类型" title="模块菜单类型"  placeholder="请输入菜单类型" value="" validation={"maxlength":"255","data-bv-stringlength-message":"菜单类型为1-255个字符!"}/>
